            ...
            <div class="bulletin">
                ...
                <ul v-if="seller.supports"
                    ...
                    <li v-for="(item, index) in seller.supports"
                        ...
                        <span class="icon"
                              v-bind:class="classMap[seller.supports[index].type]"></span>
...
export default {
    ...
    methods: {
        ...
        _initPicsScroll() {
            /* 由于ul的长度和父元素的长度一样，没有超过父元素，better-scroll是无法生效的，所以要计算它的长度并赋值给它 */
            if (this.seller.pics) {
                let picWidth = 120;
                let margin = 6;
    ...
    mounted() {
        /* 渲染完成时创建better-scroll，即通过if语句 */
        this._initScroll();
        this._initPicsScroll();
    },
    watch: {
        /* 检测从父元素里异步传递过来的数据，传到之后由于已经存在scroll，执行else语句 */
